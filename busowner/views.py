...
from busowner.serializers import BusOwnerSerializer,BusSerializer,BusCategorySerializer,BusRouteSerializer,RouteSerializer,BusRouteStopsSerializer,BusStopDetailSerializer,StopSerializer,BusRouteListSerializer,StopViewSerializer,BusOwnerProfileSerializer,BusStopDetailEditSerializer,BusEditSerializer
...
class BusView(ViewSet):
    ...
    def update(self,request,*args,**kwargs): 
        ...
        serializer=BusEditSerializer(data=request.data,instance=obj)
        if serializer.is_valid():
            serializer.save()
...
class BusRouteStopDetailView(APIView):
    authentication_classes=[authentication.TokenAuthentication]
    permission_classes=[permissions.IsAuthenticated,IsBusOwnerApproved]
    ...
    def put(self,request,*args,**kwargs): 
        id=kwargs.get("pk")
        obj=BusStopDetail.objects.get(id=id)
        serializer=BusStopDetailEditSerializer(data=request.data,instance=obj)
        if serializer.is_valid():
            serializer.save()
            return Response(data=serializer.data)
        else:
